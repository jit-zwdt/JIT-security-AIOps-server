...
public interface DictionaryItemRepo  extends JpaRepository<SysDictionaryItemEntity, String>, JpaSpecificationExecutor<SysDictionaryItemEntity> {
    ...
    @Query(value = "select * from sys_dict_item e where e.is_deleted = 0 and e.dict_id = ?1 and if(?2 != '',e.item_text like %?2%,1=1)  and if(?3 != -1,e.status = ?3,1=1) limit ?4,?5",nativeQuery = true)
